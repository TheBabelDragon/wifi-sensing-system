...
class SensingCommandListener:
    """
    Resilient command listener with exponential backoff reconnection.

    See INTEGRATION_CONTRACT.md for message contract.
    """
    ...
    def listen(self):
        while True:
            ...
            try:
                ...
                logger.warning(f"Listener error, will reconnect: {e}")
                self.pubsub = None
                time.sleep(min(self._backoff, 30))
                self._backoff = min(self._backoff * 2, 30)
