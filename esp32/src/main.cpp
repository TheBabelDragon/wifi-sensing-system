...
void makeAuthHex(char out[9], const char* node, uint32_t ts_ms) {
  snprintf(out, 9, "%08x", (unsigned)fnvTag(ECHO_SECRET, node, ts_ms / 60000UL));
}
...
void updateExcitement() {
  ...
  if (now - lastRfSampleMs >= 350) {
    uint32_t d = csiIrqCount - lastRfIrqCount;
    lastRfIrqCount = csiIrqCount;
    lastRfSampleMs = now;
    float instant = constrain(d / 36.0f, 0.0f, 1.0f);
    rfBusyness = rfBusyness * 0.4f + instant * 0.6f;
  }
  float raw =
      activityLevel * 0.30f +
      movementIntensity * 0.22f +
      rfBusyness * 0.38f +
      boostLevel * 0.12f +
      (hasFieldMirror ? fieldMotion * 0.18f : 0.0f);
  raw = constrain(raw, 0.0f, 1.0f);
  excitementLevel = constrain(powf(raw, 0.72f), 0.0f, 1.0f);
}

#if HAS_DISPLAY
const uint16_t COL_BG     = 0x0186;
...
uint16_t heatColor(float t) {
  ...
  if (t < 0.18f) {
    float u = t / 0.18f;
    return tft.color565(0, (uint8_t)(20 + u * 100), (uint8_t)(40 + u * 120));
  } else if (t < 0.40f) {
    float u = (t - 0.18f) / 0.22f;
    return tft.color565(0, (uint8_t)(160 + u * 95), 255);
  } else if (t < 0.60f) {
    float u = (t - 0.40f) / 0.20f;
    return tft.color565((uint8_t)(u * 255), (uint8_t)(255 - u * 100), 255);
  } else if (t < 0.80f) {
    float u = (t - 0.60f) / 0.20f;
    return tft.color565(255, (uint8_t)(160 - u * 100), (uint8_t)(255 - u * 255));
  }
  float u = (t - 0.80f) / 0.20f;
  return tft.color565(255, (uint8_t)(40 + u * 80), (uint8_t)(u * 40));
}
...
void drawBar(int x, int y, int w, int h, float v, uint16_t c) {
  ...
  if (fill > 0) {
    tft.fillRoundRect(x + 1, y + 1, fill, h - 2, 2, c);
    if (fill > 6) tft.fillRect(x + fill - 3, y + 2, 3, h - 4, COL_WHITE);
  }
}
...
void updateDisplay() {
  ...
  if (excitementLevel > 0.55f) {
    uint16_t border = heatColor(excitementLevel);
    tft.drawRect(0, 0, 320, 240, border);
    tft.drawRect(1, 1, 318, 238, border);
    if (excitementLevel > 0.8f) tft.drawRect(2, 2, 316, 236, COL_WHITE);
  } else {
    tft.drawRect(0, 0, 320, 240, COL_BG);
    tft.drawRect(1, 1, 318, 238, COL_BG);
    tft.drawRect(2, 2, 316, 236, COL_BG);
  }
  ...
  if (hasFieldMirror) {
    ...
    tft.setTextColor(heatColor(rfBusyness), COL_BG);
  ...
  for (int i = 0; i < 32; i++) {
    ...
    float tint = shown * 0.55f + excitementLevel * 0.45f;
    if (hasFieldMirror && fieldAgreed) tint = constrain(tint + 0.15f, 0.0f, 1.0f);
    uint16_t c = heatColor(tint);
    tft.fillRect(x, baseY - h, barW, h, c);
    if (shown > 0.55f) {
      int tip = h / 8;
      if (tip < 2) tip = 2;
      tft.fillRect(x, baseY - h, barW, tip, COL_WHITE);
    }
  }
}
#else
